...
**Period:** 2026-03-24 to 2026-05-09
**Production model:** `memvla_libero` (MemoryVLA-port to SmolVLA)
**Production branch / config:** `dev` / [`configs/memvla_libero.yaml`](configs/memvla_libero.yaml)
**Companion presentation:** `Memory SmolVLA.html` (12 slides, 2026-05-08)
...
![Three timesteps of a memoryless VLA rollout. Each frame is processed independently; there is no episodic link between timesteps.](docs/figures/deck/crops/slide-02_crop.png)
...
Implementation: [`src/memory_smolvla/memory/full_seq_bank.py`](src/memory_smolvla/memory/full_seq_bank.py) and [`src/memory_smolvla/memory/blocks.py`](src/memory_smolvla/memory/blocks.py) (on `dev` branch).

![FullSeqMemBank architecture and design decisions. The memory module sits between the frozen VLM and the action expert: (1) RETRIEVE via 2-layer cross-attention with H_t as query and the bank as K/V; (2) GATE blends current and retrieved features with a per-token sigmoid; (3) WRITE pushes detached H_t into the bank; (4) ToMe merges the most-similar adjacent pair when the bank exceeds 4 entries. Memory is read by the action expert only at L16; the other 7 cross-attention reads see unmodified H_t.](docs/figures/deck/crops/slide-05_crop.png)
...
![Eight cross-attention reads between VLM and expert. Memory is injected once, at L16, before the expert's *final* read of VLM features. The other seven cross-attention reads (L2–L14) see unmodified VLM features. We accept this 1/8 signal dilution in exchange for a single-point, minimally invasive modification.](docs/figures/deck/crops/slide-06_crop.png)
...
Implementation: [`src/memory_smolvla/training/trainer.py`](src/memory_smolvla/training/trainer.py), [`configs/memvla_libero.yaml`](configs/memvla_libero.yaml).
...
Source: [`results/sim_memory/all_memvla_libero.json`](results/sim_memory/all_memvla_libero.json) (memory ON), [`results/sim_memory/all_memvla_libero_bypass.json`](results/sim_memory/all_memvla_libero_bypass.json) (bypass).
...
![Per-suite success rates: no-memory baseline (grey) vs memvla_libero memory-on (red). Memory hurts every suite except `object` (saturated). The biggest regression; and the suite memory was specifically supposed to help most; is `libero_10`, which falls 28pp below baseline.](docs/figures/deck/crops/slide-09_crop.png)
