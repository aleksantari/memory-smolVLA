...
class MemorySmolVLAPolicy(nn.Module):
    ...
    Args:
        base_policy: A frozen ``SmolVLAPolicy`` instance. All its
            parameters will have ``requires_grad`` set to ``False``.
    ...
    def _memory_callback(
        ...
        Writes current features to the memory bank, retrieves relevant
        past context via cross-attention, and fuses via sigmoid gate.
        ...
        # Step 1: Write current prefix to memory bank (detached internally)
        for b in range(B):
        ...
        # Step 2: Advance timestamp
        current_time = self._timestamp
        self._timestamp += 1

        # Step 3: Read all memories
        # After writing, bank always has ≥1 entry.
        ...
        # Step 5: Reshape for cross-attention
        ...
        # Step 6: Cross-attention retrieval
        ...
        # Step 7: Apply zero-initialized projection
    ...
    def trainable_parameters(self) -> Iterator[nn.Parameter]:
        """Yield only the memory module parameters for the optimizer.

        The base policy parameters are all frozen.
        """
