...
# training, not a tool-watcher). Launch: nohup bash scripts/run_v9_eval_at_60k.sh > .v9_eval60k.log 2>&1 &
set -u
cd ~/memory-smolVLA
...
CFG=configs/memvla_libero_v9.yaml
OUT=results/v9_eval

echo "[v9eval] waiting for 60k (final.pt) + GPU free ... $(date -u +%H:%M)"
while true; do
  if [ -f "$CKPT" ] && ! pgrep -f "train.py --config configs/memvla_libero_v9" >/dev/null; then
    break
  fi
  sleep 300
done
echo "[v9eval] final.pt present, training done -> starting eval $(date -u +%H:%M)"
sleep 30  # let the GPU settle

echo "[v9eval] === MEM-ON $(date -u +%H:%M) ==="
python scripts/eval_memory_libero.py --checkpoint "$CKPT" --config "$CFG" \
  --all-suites --n-episodes 10 --output-dir "$OUT"

echo "[v9eval] === BYPASS $(date -u +%H:%M) ==="
python scripts/eval_memory_libero.py --checkpoint "$CKPT" --config "$CFG" \
  --all-suites --n-episodes 10 --bypass-memory --output-dir "$OUT"

echo "[v9eval] === MEM-ON + ENSEMBLE $(date -u +%H:%M) ==="
python scripts/eval_memory_libero.py --checkpoint "$CKPT" --config "$CFG" \
  --all-suites --n-episodes 10 --ensemble --query-every 10 --ensemble-decay 0.1 \
  --output-dir "${OUT}_ensemble"

echo "[v9eval] ALL_DONE $(date -u +%H:%M)"
