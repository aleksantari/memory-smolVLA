#!/usr/bin/env bash
# 1ep × 10 task canary on v5_two_stream_v4hp across all 4 suites.
set -u
...
LOG=logs/v5_two_stream_canary.log
mkdir -p logs results/v5_two_stream_v4hp_1ep
...
for SUITE in libero_spatial libero_object libero_goal libero_10; do
    ...
    "$PY" scripts/eval_memory_libero_v2.py \
        ...
        --suite "$SUITE" \
        --n-episodes 1 \
        --output-dir results/v5_two_stream_v4hp_1ep \
        >> "$LOG" 2>&1
